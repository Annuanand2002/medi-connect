...
import { IRetryDoctorRequestUseCase } from "./IRetryDoctorRequest";
...
export class RetryDoctorRequestUseCase implements IRetryDoctorRequestUseCase{
    ...
    async execute(request: RetryDoctorRequestDTO): Promise<DoctorRequest> {
        ...
        const doctorRequest = await this.doctorReqRepo.findById(retryToken.doctorRequestId)
        if(!doctorRequest){
            throw new AppError("Request not found",HTTP_STATUS.NOT_FOUND)
        }
        const {profileImg,governmentId,medicalLicense,degreeCertificates} = await this.doctorRequestFileService.uploadFiles(request);

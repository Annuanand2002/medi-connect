...
import { IRejectDoctorRequestUseCase } from "./IRejectDoctorRequest.usecase";
...
export class RejectdoctorRequestUseCase implements IRejectDoctorRequestUseCase{
    ...
    async execute(dto: RejectDoctorRequestDTO): Promise<void> {
        const doctorRequest = await this.doctorReqRepo.findById(dto.doctorRequestId);
        if(!doctorRequest){
            throw new AppError("Request not found",HTTP_STATUS.NOT_FOUND);
        }
        if(doctorRequest.status !== "PENDING"){
            throw new AppError("Only pending doctor request can be rejected",HTTP_STATUS.BAD_REQUEST)
        }
        ...
        const retryLink = canRetry?`${env.FRONTEND_URL}/doctor/retry?token=${token}`:"";

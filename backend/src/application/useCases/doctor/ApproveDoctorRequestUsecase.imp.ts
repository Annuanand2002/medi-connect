...
import { IApproveDoctorRequestUsecase } from "./IApproveDoctorRequestUsecase";
...
export class ApproveDoctorRequestUsecase implements IApproveDoctorRequestUsecase{
    ...
    const lastNumber = parseInt(
      lastDoctor.doctorCode.replace("DOC", ""),
      10
    );
    ...
    async execeute(request: ApproveDoctorRewuestDTO): Promise<void> {
        ...
        if(requests.status !== "PENDING"){
            throw new AppError("Doctor request has already been processed",HTTP_STATUS.CONFLICT)
        }
        ...
        await this.doctorReqRepo.update(request.doctorRequestId,{status:"APPROVED"})
    }
}